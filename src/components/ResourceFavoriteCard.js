...
const ResourceFavoriteCard = ({ resource }) => {
  ...
  const favoriteStar = (resourceId) =>
    isFavorite(resourceId) ? (
      <svg
        className="w-8 h-8 sm:w-9 sm:h-9 text-yellow-300"
        aria-hidden="true"
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
  ...
  return (
    <div
      className={`w-full sm:w-[220px] md:w-[250px] lg:w-[300px] ${color} flex justify-between px-4 sm:px-2 py-4 rounded-2xl items-center`}
      key={resource._id}
    >
      <a
        href={resource.url}
        target="_blank"
        className={`flex text-white/80 text-xs sm:text-sm md:text-base z-50 p-3 rounded-lg items-center w-full gap-3 cursor-pointer`}
      >
        {icon}
...
export default ResourceFavoriteCard;
